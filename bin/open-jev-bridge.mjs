...
export async function main(argv=process.argv.slice(2)){
 ...
 if(command==='doctor'){
  ...
  print({status:'ok',provider:config.provider,...(config.provider==='shisa'?{shisa_backend:config.shisaBackend}:{}),models:status.models,contract_probe:'all three response types validated',...(probe.bridge?.prompt_source?{prompt_source:probe.bridge.prompt_source,control_tokens_validated:probe.bridge.control_tokens_validated}:{}),answers:probe.answers,notice:'A successful API probe is not a model-quality or native-host certification.'});return;
 }
