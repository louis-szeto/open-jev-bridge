...
export class ToolService {
 ...
 async status(_a,signal){const result=await this.client.models({signal});return {status:'ok',model:this.config.model??'kev-latest',provider:this.config.provider??'generic',...(this.config.provider==='shisa'?{shisa_backend:this.config.shisaBackend??'vllm'}:{}),models:result.models,capabilities:{system_one:true,judgment_tools:10,compaction_library:true,claude_function_compaction:'opt-in; requires function-hook runtime',codex_history_replacement:false,codex_checkpoint_hooks:true,automatic_completion_hooks:true,automatic_patch_review:this.config.autoReview??true,event_evidence_tracking:true,automatic_external_screening:this.config.autoScreen??true},metrics:this.client.metrics??{}};}
}
