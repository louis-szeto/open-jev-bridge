...
Added in **0.4.1**. This is a separate, explicit serving backend for the existing
...
Use the 0.4.1 source in the **same directory** from which you installed the bridge.
...
# Check the actual API, template, option tokens and all three answer types.
...
plugin, refresh it with `node bin/open-jev-bridge.mjs native-claude-functions`
**instead of direct installation**, and launch Claude from a shell exporting the
same values and `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1`. The function plugin cannot
...
/mnt/md0/llama.cpp/build/bin/llama-server \
  ...
  --alias shisa-de-1 --jinja -ngl 999 -c 8192
```

`--alias` gives a readable model-list name; the native single-model routes do not
require that alias. The bridge does not alter GPU, NUMA, KV-cache or flash-attention
...
| `POST /apply-template` | `messages`, `add_generation_prompt: true`, `chat_template_kwargs: {enable_thinking: false}` | Nonempty `prompt` string |
...
For every question the served template's token IDs must equal those of Shisa's
published scaffold. Each option letter must tokenize to one distinct token, and
`prompt + letter` must preserve all prompt token IDs and append only that token.
The full prompt plus one prediction must fit the smaller of the server's per-slot
limit and the bridge budget. Four slots of 8,192 tokens are **not** one 32,768-token
context. Server-side truncation, changed prompt counts, missing probability rows,
invalid IDs or a different template all fail explicitly.
...
`--props` configuration. A template mismatch must be resolved with the correct
GGUF template/Jinja setup, not by disabling the comparison. Unsupported older
...
vLLM-shaped request. It emulates llama.cpp's native envelopes and computes a
...
sequences and the ready-to-use Claude function bundle.
