import fs from 'node:fs/promises';import os from 'node:os';import path from 'node:path';import {performance} from 'node:perf_hooks';import assert from 'node:assert/strict';
import {fileURLToPath} from 'node:url';import {SystemOneClient} from '../src/client.mjs';import {resolveConfig} from '../src/config.mjs';import {compact} from '../src/compact.mjs';import {evaluateBelay} from '../src/belay.mjs';import {parseAnswer} from '../src/schema.mjs';
import {mockSystemOne,mcpProcess,history,verified,envelope,command,BIN,temp} from '../tests/helpers.mjs';
...
try{
 ...
 results.push(await measure('verified_belay_fast_path_no_model',1000,20,()=>evaluateBelay(verified(),undefined,()=>{throw Error('No model call allowed');})));
 const pq={type:'choice',instructions:'pick',criteria:Object.fromEntries(Array.from({length:255},(_,i)=>[String(i),null]))},pa={type:'choice',choice:'0',confidence:0,probabilities:Object.fromEntries(Object.keys(pq.criteria).map(k=>[k,0]))};
 results.push(await measure('validate_255_rounded_options',1000,20,()=>assert.ok(parseAnswer(pq,pa).ok)));
 home=await temp();const transcript=path.join(home,'transcript.jsonl');await fs.writeFile(transcript,verified().map(x=>JSON.stringify(x)).join('\n'));
 ...
 const report={generated_at:new Date().toISOString(),mode:live?'LIVE_SYSTEM_ONE':'OFFLINE_LOOPBACK_FIXTURE_NOT_MODEL_INFERENCE',interpretation:live?'Measures this configured System One deployment including network and inference; no cross-provider comparison.':'HTTP/MCP timings measure bridge plus deterministic local HTTP fixture; compaction uses fixed answers. They are NOT model inference latency, model accuracy or real-workload token savings.',environment:{node:process.version,platform:process.platform,arch:process.arch,cpu:os.cpus()[0]?.model,logical_cpus:os.cpus().length,total_memory_bytes:os.totalmem(),rss_bytes:process.memoryUsage().rss,max_rss_kib:process.resourceUsage().maxRSS},models:models.models,results,compaction:{...compactResult.stats,input_messages:historyInput.length,output_messages:compactResult.messages.length,metric:'serialized canonical characters, not actual model tokens',fixture_decisions:!live?'all unpinned tool pairs dropped':undefined}};
 await fs.mkdir(path.join(root,'reports'),{recursive:true});const file=path.join(root,'reports',live?'benchmark-live.json':'benchmark.json');await fs.writeFile(file,JSON.stringify(report,null,2)+'\n');
 console.log(`${report.mode}\n${file}`);for(const r of results)console.log(`${r.name}: p50=${r.p50_ms.toFixed(3)}ms, p95=${r.p95_ms.toFixed(3)}ms, ${r.throughput_per_second.toFixed(1)} ops/s`);console.log(`Compaction fixture character reduction: ${(compactResult.stats.reductionRatio*100).toFixed(2)}%`);
}catch(e){console.error(`Benchmark FAILED: ${e.message}`);process.exitCode=1;}finally{if(mcp)await mcp.close();if(server)await server.close();if(home)await fs.rm(home,{recursive:true,force:true});}
