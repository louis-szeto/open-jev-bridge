...
export async function buildPlugins(){
 ...
 await fs.writeFile(path.join(target,'package.json'),JSON.stringify({name:'open-jev-bridge-functions',version:'0.4.4',private:true,type:'module',engines:{node:'>=22.16.0'}},null,2)+'\n');
 await fs.writeFile(path.join(target,'.claude-plugin','plugin.json'),JSON.stringify({name:'open-jev-bridge-functions',version:'0.4.4',description:'Opt-in configured System One verbatim compaction via Claude function hooks, completion Stop hook, and MCP tools',license:'MIT',hooks:['./hooks/commands.json','./hooks/functions.json']},null,2)+'\n');
