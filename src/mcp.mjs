...
export async function serveMcp(service,{input=process.stdin,output=process.stdout,maxLineBytes=4000000,maxPending=16}={}){
 ...
 const dispatch=async message=>{
  ...
  if(method==='initialize'){
   ...
   initialized=true;await send({jsonrpc:'2.0',id,result:{protocolVersion:MCP_VERSIONS.includes(params.protocolVersion)?params.protocolVersion:MCP_VERSIONS[0],capabilities:{tools:{listChanged:false}},serverInfo:{name:'open-jev-bridge',version:'0.4.1'},instructions:'Provider-backed probabilistic judgments, not proof. Tools never execute commands. Use actual tests to establish completion.'}});return;
  }
